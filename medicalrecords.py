...
def create_medical_records(connection):
    connection.cursor().execute("DROP TABLE MedicalRecord")
    create_table_query = """
...
def input_data_medical_records(connection, fake):
    connection.cursor().execute("SELECT id FROM Pasien")
    PasienID = connection.cursor().fetchall()
    x = PasienID.count()
    ...
    for i in range(x):
        ...
        checkin_date = fake.date_between_dates(min_date=min_checkin_date, max_date=max_checkin_date)
    ...
    for i in range(50):
        ...
        # Calculate the check-out date
        checkout_date = checkin_date + timedelta(days=length_of_stay)
        id_pasien = PasienID[random.choice() % x-1]['id']
        id_kerja_sama = x+i+1
        diagnosis = fake.sentence()
        biaya = fake.pydecimal(left_digits=3, right_digits=2, positive=True, min_value=10)
        with connection.cursor() as cursor:
