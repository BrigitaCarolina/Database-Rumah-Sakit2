...
def input_departemen(connection, fake):
    ...
    for i in range(len(nama_departemen)):
        ...
        departemen = nama_departemen[random.choice() % len(nama_departemen)-1]  
        with connection.cursor() as cursor:
            cursor.execute(f"INSERT INTO departmen (nama_departemen, kepala_departemen) VALUES ('{departemen}', '{kepala_departemen}')")
